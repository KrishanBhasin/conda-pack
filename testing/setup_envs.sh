...
if [ ! -d $croot/conda-meta ]; then
    ${CONDA_EXE:-conda} create -y -p $croot conda python=3.8
fi
...
echo Creating py38 environment
env=$envs/py38
conda env create -f $ymls/py38.yml -p $env
...
echo Creating py38_missing_files environment
env=$envs/py38_missing_files
conda env create -f $ymls/py38.yml -p $env
if [ -f $env/python.exe ]; then
    rm $env/lib/site-packages/toolz/*.py
else
    rm $env/lib/python3.8/site-packages/toolz/*.py
fi
...
echo Creating py38_editable environment
env=$envs/py38_editable
conda env create -f $ymls/py38.yml -p $env
...
echo Creating py38_broken environment
env=$envs/py38_broken
conda env create -f $ymls/py38_broken.yml -p $env
